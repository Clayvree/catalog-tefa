                        ...
                        <template x-for="(task, index) in tasks" :key="index">
                            <div class="bg-white p-4 rounded-lg border border-gray-200 shadow-sm mb-4 relative">
                                ...
                                <div class="flex items-center gap-2 bg-indigo-50 px-3 py-2 rounded text-xs">
                                    ...
                                    <input type="text" :name="`tasks[${index}][reasoning]`" x-model="task.reasoning" class="w-full border-0 focus:ring-0 p-0 text-xs bg-transparent text-indigo-600" placeholder="Kenapa tugas ini dibuat?">
                                </div>
                            </div>
                        </template>
                    </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('waReview', () => ({
        ...
        addTask() {
            this.tasks.push({ title: '', instructions: '', reasoning: '' });
        },
        removeTask(index) {
